...
parser.add_argument('--template_threshold', type=float, default=2, help='threshold for determining whether to update template')
...
template_org_rect = [280, 152, 330, 318]
template_cur = seq[:,:,0]   # Tn
template_cur_rect = [280., 152., 330., 318.]
pn = np.zeros(2)
pn_str = np.zeros(2)
...
np.save('girlseqrects-wcrt.npy',rect_array)

# Display rectangles
rect_array = np.load('girlseqrects-wcrt.npy')
rect_array_old = np.load('girlseqrects.npy')
